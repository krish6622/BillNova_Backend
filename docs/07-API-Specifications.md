...
| Method | Path | Role | Description |
|--------|------|------|-------------|
| GET | `/settings` | O | Business profile + invoice + GST defaults (incl. `show_gst_on_invoice`, CR-2). |
| PUT | `/settings` | O | Update profile, invoice prefix/footer, gst_mode_default, place_of_supply, `show_gst_on_invoice` (CR-2). |
...
## 7.15 CR-2 — Pricing & Invoice GST API delta

Supersedes the relevant parts of §7.5 / §7.8 / §7.12. (See [13-FRD-ChangeRequest-CR2.md](13-FRD-ChangeRequest-CR2.md).)

**Products — `/api/products`** (created via purchases; CR-1):
- `ProductOut` now returns **`profit_amount`** (NUMERIC ≥ 0) instead of `margin_type` / `margin_value`.
- `selling_price` is **read-only, derived**: `selling_price = round(purchase_price + profit_amount, 2)`.
- `PUT /products/{id}` accepts `profit_amount` (≥ 0); supplying it recomputes `selling_price`. `margin_type`/`margin_value` are **removed** and rejected as unknown fields.
```json
// PUT /products/{id}  — editable subset
{ "name": "Cotton Saree", "gst_percentage": 5, "hsn_code": "5407",
  "reorder_level": 5, "profit_amount": 25, "is_active": true }
// → selling_price recomputed = purchase_price + 25
```

**Purchases — `POST /api/purchases`** (inline pricing per line):
- Each item replaces `margin_type`/`margin_value` with **`profit_amount`** (≥ 0). `purchase_price` must be **> 0**.
```json
{ "supplier_name": "Mills Co", "purchase_date": "2026-06-10",
  "items": [ { "product_name": "Cotton Saree", "hsn_code": "5407", "gst_percentage": 5,
               "purchase_price": 600.00, "profit_amount": 150.00, "quantity": 100, "unit": "NOS" } ] }
// → product selling_price = 750.00
```

**Settings — `/api/settings`**:
- `SettingsOut` adds **`show_gst_on_invoice: boolean`** (default **false** = Hide). `SettingsUpdate` accepts it (optional).

**Invoice — `GET /api/sales/{id}/invoice`**:
- `business` block adds **`show_gst_on_invoice: boolean`** (from the tenant). Frontend renders **Hide** (item/qty/rate/amount + grand total only, label "INVOICE") or **Show** (full tax breakup, label "TAX INVOICE").
- **Statutory invariant:** sale records always persist full tax (`taxable_value`, `cgst`, `sgst`, `igst`, `total_gst`); the toggle changes **rendering only**. GST/HSN/auditor reports ignore it entirely.

**GST source (locked, CR-2.1):** customer/output GST is always computed from `selling_price`; `purchase_price` is used only for purchase/input tax and stock valuation — never for a sale line.
